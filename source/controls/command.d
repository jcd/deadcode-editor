...
import deadcode.api.api : ICommand;
import deadcode_application;
import deadcode.core.command : CommandManager, Command, CompletionEntry, Hints;
...
import deadcode.gui.controls.textfield;
...
import deadcode.gui.styledtext;
...
//	}
	...
	this(Widget parent, float _height, BufferView bufView, DeadcodeApplication _app)
	{
		super(parent);
		app = _app;
...
//        commandContainer.features ~= new GridLayout(GridLayout.Direction.column, 2);
		...
		auto completionView = app.newBufferView();
		...
		onKeyboardFocusCallback = (Event ev, Widget w) {
			app.currentBufferView = commandField.bufferView;
	...
	override EventUsed onCommandEvent(CommandEvent event)
	{
		...
		case "navigate.down":
			...
			return EventUsed.yes;
		case "navigate.right":
			import deadcode.edit.buffer : TextBoundary;
	...
	void cycleBuffers(int i)
	{
		...
		if (completions.length)
        {
	    	    ...
	    	    app.previewBufferView(completions[cycleBufferStartOffset].data);
        }
        else
        {
            cycleBufferStartOffset = 0;
...
version(oldvw)
{
	...
	private void showCommandArgumentCompletions(ICommand cmd, CompletionEntry[] compls)
	{
		if (compls.empty)
		{
			bool showCommandCompletion = true; // (cmd.hints & Hints.completion) != 0;
	...
	private void completeCommandName(string str)
	{
		...
		auto cmdNameSearch = munch(str, "^ ");
		ICommand[] cmdList = app.commandManager.lookupFuzzy(cmdNameSearch);
	...
	private void completeCommandArgument(ICommand cmd, string str)
	{
...
version(NONE)
{
	...
	void executeCommand()
	{
		...
		if (cmdData.cmd is null)
		{
			...
			if (cmdData.cmd is null)
			{
				app.log.info("Unknown command: %s", cmdData.rest);
				return;
			}
		}
	...
	void onMissingCommandArguments(Command cmd, CommandParameter[] args)
	{
		...
		auto str = getCommandArgsString(cmd);
		// displayStringList([str]);
		app.log.info("Missing arguments: %s", str);
	}

	private string getCommandArgsString(ICommand cmd)
	{
